export function cleanLyrics(text) {
  ...
  return lines
    .map((line) => {
      ...
      // Remove one or many LRC timestamps at line start: [mm:ss.xx], <mm:ss.xx>, or mm:ss.xx
      cleaned = cleaned.replace(/^(?:\[\d{1,2}:\d{2}(?:[.:]\d{1,3})?\]\s*)+/g, "");
      cleaned = cleaned.replace(/^(?:<\d{1,2}:\d{2}(?:[.:]\d{1,3})?>\s*)+/g, "");
      cleaned = cleaned.replace(/^(?:\d{1,2}:\d{2}(?:[.:]\d{1,3})?\s*)+/g, "");
      // Remove remaining inline timestamp tokens from badly formatted rows.
      cleaned = cleaned.replace(/\s*(?:\[\d{1,3}:\d{2}(?:[.:]\d{1,3})?\]|<\d{1,3}:\d{2}(?:[.:]\d{1,3})?>)\s*/g, " ");
