...
declare
  ...
  v_wpm integer := greatest(0, coalesce(p_wpm, 0));
  ...
  v_duration_seconds integer := greatest(0, coalesce(p_duration_seconds, 0));
  v_level integer;
  v_prestige integer;
...
begin
  ...
  xp_base := greatest(
    10,
    round((35 + (least(v_wpm, 130) * 0.55) + (v_accuracy * 0.45)) * v_mode_mult)
  );
  ...
  insert into public.game_results (
    user_id, song_title, artist, mode, wpm, accuracy,
  ...
  values (
    v_uid, nullif(trim(coalesce(p_song_title, '')), ''), nullif(trim(coalesce(p_artist, '')), ''), v_mode,
...
revoke all on function public.apply_game_progress(text, text, text, integer, integer, integer, integer, integer, integer, integer, integer) from public;
grant execute on function public.apply_game_progress(text, text, text, integer, integer, integer, integer, integer, integer, integer, integer) to authenticated;
